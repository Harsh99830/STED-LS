...
function ConceptLearned() {
  ...
  const handleAddConcepts = async () => {
    ...
    try {
      const updatedLearnedConcepts = [...learnedConcepts, ...selected];
  ...
  return (
    <div className='text-left'>
                ...
                {isOpen && (
                  ...
                  >
                    <div className="border-t border-slate-200 pt-3">
                      <ul className="pl-4 list-disc list-inside text-slate-600 space-y-1">
                        {categoryLearnedConcepts.length > 0 ? (
                          categoryLearnedConcepts.map((item) => <li key={item.concept}>{item.concept}</li>)
                        ) : (
                          <li className="list-none italic text-slate-400">No concepts learned in this category yet.</li>
                        )}
                      </ul>
                    </div>
                  </motion.div>
      ...
      <button
        className='border border-slate-300 rounded-md py-1 px-2 mt-4 cursor-pointer'
